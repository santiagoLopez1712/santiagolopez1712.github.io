...
(function() {
  ...
  const styles = `
    ...
    .n8n-chat-widget .chat-input textarea {
      ...
      background: rgba(255,255,255,0.9); color: var(--chat--color-font);
    ...
    /* Canvas del visualizador dentro del input */
    ...
    /* Estado activo del mic (si usas SVG con stroke) */
  ...
  // Ajuste canvas al tamaño del input
  ...
  function drawWave(){
    ...
    // respirar con nivel RMS
    let sum=0; for (let i=0;i<dataArray.length;i++){ const d=dataArray[i]-128; sum+=d*d; }
  ...
  if (!SpeechRecognition) {
    ...
    recognition.continuous = true;     // mantener escucha
    ...
    recognition.onend = () => {
      if (!manualStop) {
        recognition.start(); // reintenta para continuidad
        return;
      }
      micButton.classList.remove('active');
      micButton.setAttribute('aria-pressed', 'false');
